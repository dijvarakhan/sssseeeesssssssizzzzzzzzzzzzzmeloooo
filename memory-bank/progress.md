...
- **NEW: /radio command** — Added a grid-based radio station selector with 30+ Turkish stations.
- **NEW: Message Auto-deletion** — `/oynat` and `/radio` commands now delete the user's trigger message to keep chat history clean.
- **NEW: Enhanced /cookie** — Sudo users can now update cookies via file upload or raw text in private messages.
